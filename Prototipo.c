	...
	switch(pid)
	{
		...
		case 0:
			...
			else 
			{
				...
				send(sock_conectado, "Bienvenido al servidor. Digite <Enter> para iniciar o <Salir> para terminar la conversacion. \n",95,0);
				//buffer[i]='\0';
				...
				printf(/*Rojo "cliente: " ResetColor*/ Rojo "%s\n" ResetColor, buffer1);
...
void cliente (char *texto[],int puerto)    //int argc, char *texto[] !IP,Puerto!
	...
	while((salidaServidor!=0) && (salidaCliente !=0))
	
		{
					...
					printf(/*"servidor:*/ Rojo "%s" ResetColor, buffer);
					for(i=0;i<=255;i++) buffer[i]=0;
					}
